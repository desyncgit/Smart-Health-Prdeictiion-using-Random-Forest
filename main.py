...
class DiseasePrediction:
    ...
    def _load_train_dataset(self):
        ...
        print("Length of features in the first row:", len(train_features.iloc[0]))
        # Check for data sanity
        assert (len(train_features.iloc[0]) == 131)
    ...
    def select_model(self):
        ...
        elif self.model_name == 'random_f':
            self.clf = RandomForestClassifier(n_estimators=self.config['model']['random_f']['n_estimators'])
...
if __name__ == "__main__":
    ...
    dp = DiseasePrediction(current_model_name)
    # Train the Model
    dp.train_model()
    # Get Model Performance on Test Data
    test_accuracy, classification_report = dp.make_prediction('random_f')
